...
var connected_on_lan bool = true
...
// Log_write
func Log_Write( hostResponse *string, f *os.File ) {


	if _, err := f.WriteString( *hostResponse ); err != nil {
...
func Get_Response(host *string, context *string) string {
	...
	if connected_on_lan {
		...
		response = "Test line" + strconv.FormatInt( time.Now().Unix(), 10 )
	}
